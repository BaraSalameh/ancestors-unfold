...
`/member/$id`, and `/branches`. `/subfamilies` remains a compatibility redirect
to `/branches` and preserves validated `treeId` and `branchId` search values.
...
| Active tree graph       | `family_trees`, `tree_memberships`, `family_members`, `subfamilies`, parent-child relationships, `unions`, `union_partners`, `external_children`, `branch_grants`, `files`, `subfamily_attachments` | Snapshot read/write SQL, branch attachment handlers, contributor policies, RLS helpers, integrity triggers                |
| Active collaboration    | `contributor_invitations`, `ownership_transfers`, `tree_activity`, `member_contacts`, `authenticity_config`                                                                                         | Collaboration endpoints, dashboard/activity contracts, contact authorization, authenticity queries                        |
...
| Deprecation candidates  | TOTP/recovery, device-login challenges, share links, ownership history, change requests, complaints                                                                                                 | Runtime SQL search, RLS/policy search, trigger search, DB tests, operational scripts, and a production deprecation period |
...
TOTP recovery storage, share-link storage, ownership history, member-change
requests, and complaints. File/media tables, persisted subfamily attachments,
and authenticity configuration are active dependencies of the branch workspace.
No object is approved for removal by this document.
