  ...
  try {
    ...
    if (url.pathname === "/api/ready") {
      if (!databaseConfigured)
        return json({ status: "not_ready", reason: "DATABASE_URL is not configured" }, 503);
      const r = await query<{ count: string }>(
          "SELECT count(*)::text count FROM public.schema_migrations",
        ),
        required = Number(process.env.REQUIRED_MIGRATIONS ?? 3);
      return Number(r.rows[0]?.count) >= required
        ? json({ status: "ready", migrations: Number(r.rows[0].count) })
        : json(
            {
              status: "not_ready",
              reason: "migrations missing",
              required,
              applied: Number(r.rows[0]?.count ?? 0),
            },
            503,
          );
    }
