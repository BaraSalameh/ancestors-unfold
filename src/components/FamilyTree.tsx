...
import { SubfamilyPanel } from "@/features/subfamilies/components/subfamily-panel";
import { routeParentEdges } from "@/features/trees/domain/route-edges";
...
function Inner({ readOnly = false }: { readOnly?: boolean }) {
  ...
  const onConnect = useCallback(
    (conn: Connection) => {
      ...
      toast.success(`${displayName(parent, lang)} â†’ ${displayName(child, lang)}`);
    },
    [t, lang],
  );
  ...
  const pickMother = (wifeId: string | null) => {
    ...
    if (father && child) {
      toast.success(`${displayName(father, lang)} â†’ ${displayName(child, lang)}`);
    }
