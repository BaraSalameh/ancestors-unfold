  ...
  return <main className="min-h-[calc(100vh-3.5rem)] bg-muted/25">
    ...
    <section className="mx-auto max-w-7xl px-4 py-9 sm:px-6 lg:px-8">
      ...
      {filtered.length ? <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">{filtered.map((tree) => <article key={tree.id} className="group overflow-hidden rounded-xl border bg-card shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
        <div className={`relative h-32 bg-gradient-to-br ${tree.color}`}><div className="absolute inset-0 opacity-20" style={{ backgroundImage: "radial-gradient(circle at 20% 20%, white 0 2px, transparent 3px), radial-gradient(circle at 65% 55%, white 0 3px, transparent 4px)", backgroundSize: "52px 52px" }} /><GitBranch className="absolute bottom-4 start-5 h-9 w-9 text-white/90" /><DropdownMenu><DropdownMenuTrigger asChild><Button size="icon" variant="secondary" className="absolute end-3 top-3 h-8 w-8 bg-card/90 text-card-foreground hover:bg-card"><MoreHorizontal className="h-4 w-4" /></Button></DropdownMenuTrigger><DropdownMenuContent align="end"><DropdownMenuItem onClick={() => openEdit(tree)}><Pencil className="me-2 h-4 w-4" />{t("rename_update")}</DropdownMenuItem><DropdownMenuItem onClick={() => setDeleting(tree)} className="text-destructive"><Trash2 className="me-2 h-4 w-4" />{t("delete")}</DropdownMenuItem></DropdownMenuContent></DropdownMenu></div>
        <div className="p-5"><h3 className="text-lg font-semibold">{lang === "ar" ? tree.name_ar || tree.name_en : tree.name_en}</h3><p className="mt-1 line-clamp-2 h-10 text-sm text-muted-foreground">{lang === "ar" ? tree.description_ar || tree.description_en : tree.description_en || tree.description_ar}</p><div className="mt-4 flex gap-4 text-xs text-muted-foreground"><span className="flex items-center gap-1"><Users className="h-3.5 w-3.5" />{tree.members} {t("members_count")}</span><span className="flex items-center gap-1"><GitBranch className="h-3.5 w-3.5" />{tree.generations} {t("generations_count")}</span></div><div className="mt-4 flex items-center justify-between border-t pt-4"><span className="flex items-center gap-1 text-xs text-muted-foreground"><CalendarDays className="h-3.5 w-3.5" />{tree.updatedAt}</span><div className="flex gap-1"><Button asChild size="sm" variant="ghost"><Link to="/tree/$id" params={{ id: tree.id }} search={{ mode: "preview" }}><Eye className="me-1 h-4 w-4" />{t("preview")}</Link></Button><Button asChild size="sm"><Link to="/tree/$id" params={{ id: tree.id }} search={{ mode: "edit" }}>{t("edit")}<ArrowUpRight className="ms-1 h-3.5 w-3.5" /></Link></Button></div></div></div>
    ...
    <Dialog open={createOpen || !!editing} onOpenChange={(open) => { if (!open) { setCreateOpen(false); setEditing(null); } }}><DialogContent><DialogHeader><DialogTitle>{editing ? t("update_family_tree") : t("create_tree_title")}</DialogTitle><DialogDescription>{editing ? t("update_tree_desc") : t("create_tree_desc")}</DialogDescription></DialogHeader><div className="grid gap-4 sm:grid-cols-2"><div><label className="mb-1.5 block text-sm font-medium">{t("family_name_en")}</label><Input autoFocus dir="ltr" value={nameEn} onChange={(e) => setNameEn(e.target.value)} placeholder="e.g. The Williams Family" /></div><div><label className="mb-1.5 block text-sm font-medium">{t("family_name_ar")} <span className="font-normal text-muted-foreground">{t("optional")}</span></label><Input dir="rtl" value={nameAr} onChange={(e) => setNameAr(e.target.value)} placeholder="مثال: عائلة ويليامز" /></div><div><label className="mb-1.5 block text-sm font-medium">{t("description_en")} <span className="font-normal text-muted-foreground">{t("optional")}</span></label><Input dir="ltr" value={descriptionEn} onChange={(e) => setDescriptionEn(e.target.value)} placeholder="A short note about this tree" /></div><div><label className="mb-1.5 block text-sm font-medium">{t("description_ar")} <span className="font-normal text-muted-foreground">{t("optional")}</span></label><Input dir="rtl" value={descriptionAr} onChange={(e) => setDescriptionAr(e.target.value)} placeholder="ملاحظة قصيرة عن هذه الشجرة" /></div></div><DialogFooter><Button variant="outline" onClick={() => { setCreateOpen(false); setEditing(null); }}>{t("cancel")}</Button><Button onClick={submit} disabled={!nameEn.trim()}>{editing ? t("save_changes") : t("create_tree")}</Button></DialogFooter></DialogContent></Dialog>
    <AlertDialog open={!!deleting} onOpenChange={(open) => !open && setDeleting(null)}><AlertDialogContent><AlertDialogHeader><AlertDialogTitle>{t("delete_tree_title")} {deleting && (lang === "ar" ? deleting.name_ar || deleting.name_en : deleting.name_en)}</AlertDialogTitle><AlertDialogDescription>{t("delete_tree_desc")}</AlertDialogDescription></AlertDialogHeader><AlertDialogFooter><AlertDialogCancel>{t("cancel")}</AlertDialogCancel><AlertDialogAction className="bg-destructive text-white hover:bg-destructive/90" onClick={() => { if (deleting) { familyStore.deleteTreeData(deleting.id); saveTrees(trees.filter((tree) => tree.id !== deleting.id)); } setDeleting(null); }}>{t("delete_family_tree")}</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
  </main>;
}
